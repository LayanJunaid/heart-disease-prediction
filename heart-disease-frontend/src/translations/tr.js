export default {

  home:"Ana Sayfa",
  test:"Test",

  homeTitle:
  "Kalp Hastalığı Tahmin Sistemi",

  homeDesc:
`Bu platform, kullanıcı tarafından girilen tıbbi ve sağlık verilerine dayanarak kalp hastalığı riskini tahmin etmek için geliştirilmiş yapay zeka tabanlı bir Kalp Hastalığı Tahmin Sistemidir.

Tahmin süreci, tıbbi tahmin araştırmalarında yaygın olarak kullanılan UCI Kalp Hastalığı Veri Seti ile eğitilmiş makine öğrenmesi algoritmalarına dayanmaktadır.

Sistem; yaş, kolesterol seviyesi, tansiyon, kalp atış hızı ve diğer önemli sağlık göstergelerini analiz ederek olası kalp hastalığı risk seviyeleri hakkında ilk değerlendirmeyi sunar.

Bu projenin amacı, yapay zeka teknolojilerinin erken sağlık riski değerlendirmesinde ve tıbbi karar verme süreçlerinin desteklenmesinde nasıl kullanılabileceğini göstermektir.

Lütfen bu sistemin yalnızca eğitim ve araştırma amaçlı olduğunu unutmayın. Sonuçlar her zaman tamamen doğru olmayabilir ve profesyonel tıbbi teşhis veya doktor danışmanlığının yerine geçmez.`,

  signIn:"Giriş Yap",
  signUp:"Kayıt Ol",

  signInDesc:
  "Kayıtlı tahmin geçmişinize ve profilinize erişmek için giriş yapın.",

  signUpDesc:
  "Tahmin sonuçlarınızı kaydetmek ve profilinizi yönetmek için hesap oluşturun.",

  email:"E-posta",
  password:"Şifre",

  fullName:"Ad Soyad",

  createAccount:"Hesap Oluştur",

  profile:"Profil",

  history:"Geçmiş",

  editProfile:"Profili Düzenle",

  saveChanges:"Değişiklikleri Kaydet",

  saveHistory:
  "Tahmin geçmişinizi kaydetmek ister misiniz?",

  saveHistoryDesc:
  "Sonuçlarınızı kaydetmek ve daha sonra erişmek için giriş yapın veya hesap oluşturun.",

  startTest:"Teste Başla",

  continueGoogle:
  "Google ile Devam Et",

  footerTitle:
  "Kalp Hastalığı Tahmin Sistemi",

  footerDesc1:
  "Kalp hastalığı riskinin erken değerlendirilmesi için yapay zeka tabanlı araç.",

  footerDesc2:
  "Bu sistem yalnızca eğitim amaçlıdır ve profesyonel tıbbi tavsiyenin yerine geçmez.",

  importantLinks:
  "Önemli Bağlantılar",

  githubRepo:
  "GitHub Deposu",

  dataset:
  "UCI Kalp Hastalığı Veri Seti",

  teamMembers:
  "Takım Üyeleri",

  advisor:
  "Danışman",

  logout:
  "Çıkış Yap",

  viewMore:
  "Daha Fazla",

  viewLess:
  "Daha Az",

  resources:"Proje Kaynakları",

  submit:"Gönder",

  invalid:"Geçersiz değer",

  testTitle:
  "Lütfen aşağıdaki bilgileri dikkatlice doldurun",

  age:"Yaş",

  ageDesc:
  "Yaşınızı yıl olarak girin.",

  sex:"Cinsiyet",

  sexDescription:
  "0 = Kadın , 1 = Erkek",

  cp:"Göğüs Ağrısı Türü",

  cpDesc:
  "Doktor tarafından belirlenen göğüs ağrısı türünü girin.",

  trestbps:"Dinlenme Kan Basıncı",

  trestbpsDesc:
  "Dinlenme sırasındaki kan basıncı değerini girin.",

  chol:"Kolesterol",

  cholDesc:
  "Kolesterol seviyenizi girin.",

  fbs:"Açlık Kan Şekeri",

  fbsDesc:
  "0 = Normal , 1 = Yüksek",

  restecg:"EKG",

  restecgDesc:
  "Dinlenme EKG sonuçlarını girin.",

  thalach:"Maksimum Kalp Hızı",

  thalachDesc:
  "Egzersiz sırasında ulaşılan en yüksek kalp hızını girin.",

  exang:"Egzersiz Anjinası",

  exangDesc:
  "0 = Hayır , 1 = Evet",

  oldpeak:"Old Peak",

  oldpeakDesc:
  "Egzersize bağlı ST düşüş değerini girin.",

  slope:"Eğim",

  slopeDesc:
  "ST segment eğim değerini girin.",

  ca:"CA",

  caDesc:
  "Floroskopi ile görülen büyük damar sayısı.",

  thal:"Thal",

  thalDesc:
  "Talasemi test sonucu kategorisi.",

  result:"Tahmin Sonucu",

  highRisk:
  "Yüksek düzeyde kalp hastalığı riski tespit edildi.",

  mediumRisk:
  "Orta düzeyde kalp hastalığı riski tespit edildi.",

  lowRisk:
  "Düşük düzeyde kalp hastalığı riski tespit edildi.",

  resultNote:
  "NOT: Tahmin edilen risk, geçmiş veri desenleri üzerinde eğitilmiş bir makine öğrenmesi modeli tarafından oluşturulmuştur. İlk değerlendirme sağlamasına rağmen tamamen doğru olmayabilir ve kesin tıbbi teşhis olarak değerlendirilmemelidir. Daha fazla değerlendirme için bir sağlık uzmanına danışmanız şiddetle tavsiye edilir.",

  footerAbout:
  "Kalp hastalığı risk değerlendirmesi için yapay zeka tabanlı eğitim amaçlı sistem.",

  disclaimer:
  "Bu sistem yalnızca eğitim amaçlıdır ve profesyonel tıbbi tavsiye veya teşhisin yerine geçmez.",

  range: "Aralık",
  ageAccuracyWarning: "Daha doğru tahminler için 28 yaş ve üzeri önerilir.",

  accountCreated: "Hesap başarıyla oluşturuldu! Lütfen giriş yapın.",
  signupFailed: "Hesap oluşturma başarısız, lütfen tekrar deneyin.",
  googleFailed: "Google ile giriş başarısız, lütfen tekrar deneyin.",
  serverError: "Sunucuya bağlanırken hata oluştu, lütfen daha sonra tekrar deneyin.",

};